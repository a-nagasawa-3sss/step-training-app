...
function renderCourseList(list) {
  ...
  list.forEach((course) => {
    ...
    meta.className = "meta";
    meta.innerHTML = `<div class="name">${escapeHtml(course.title)}</div><div class="sub">${escapeHtml(course.subtitle || "")}${course.sourceFilename ? " ／ " + escapeHtml(course.sourceFilename) : ""}</div>`;
...
async function onGenerateClick() {
  ...
  generateButton.disabled = true;
  statusBox.className = "";
  statusBox.textContent = "Geminiで問題を生成中です。しばらくお待ちください...";
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ markdown: mdText, filename: mdFilename }),
    });
